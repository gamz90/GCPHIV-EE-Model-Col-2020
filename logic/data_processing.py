...
class DataProcessing(object):
    def __init__(self, name_list: list, group_name: str = 'ALL', stochastic: bool = False, discount_rate: bool = False,
                 switch_cost: str = None, pregnancy_model: bool = False):
        result_list = list()
    ...
    def generate_dispersion(self, language: str = 'eng', n_iterations: int = 50000, has_discount_rate: bool = False):
        ...
        results = results[[discount_rate, medication_name, group, qaly, costs, acute_events, chronic_events,
                           treatment]].groupby([discount_rate, medication_name, group]).mean().reset_index(drop=False)
        ...
        print('Exported:', graph_name, dt.datetime.now())
        plt.clf()
        g = sns.pairplot(data=results[[medication_name, qaly, costs, acute_events, chronic_events, failure,
                                       treatment]], hue=medication_name, markers=mark, corner=True)
        g.add_legend()
            ...
            for gr in groups:
                for dr in discount_rates:
                    insert_threshold.append({threshold: val_to_add, probability: 0,
                                             medication_name: str(pib_value) + ' ' + pib_name, group: gr,
                                             discount_rate: dr})
