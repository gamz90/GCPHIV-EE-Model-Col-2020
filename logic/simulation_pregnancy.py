...
class Model(object):
    ...
    def calculate_probabilities(self, medication_name: str, scenario: dict):
        ...
        if probabilities is not None:
            ...
            for cond in pregnant_df['NAME'].unique():
                ...
                probability *= (1 - weights[cond])
            probability = 1 - probability
            for cond in probability['NAME'].unique():
                if probability > 0:
                    weights[cond] = weights[cond] / probability
                adverse_df = self.adverse_info[self.adverse_info['NAME'] == cond]
                for k in acute:
    ...
    def calculate_qaly_cost(self, viral_charge: bool, age: str, chronic: int, acute: bool, step_length: int,
        ...
        """
        :param integrate_pregnancy_risk: if there is a need to include the cost and QALY penalty from a pregnancy related event
        ...
        """
        qaly = self.general_info['values']['base_qaly'][age][scenario.get('base_qaly', 'BASE_VALUE')]/12
        cost = self.medications[medication_name]['values']['test_cost']['ALL'][scenario.get('test_cost', 'BASE_VALUE')]
        if viral_charge:
            qaly *= self.general_info['values']['high_test_qaly']['ALL'][scenario.get('high_test_qaly',
                                                                                                    'BASE_VALUE')]
        ...
        if integrate_pregnancy_risk:
            qaly *= 1-adverse_information['pregnancy']['Immediate_QALY']
            cost += adverse_information['pregnancy']['Immediate_Cost']
        if step_length > 6:
            cost = cost*2
        if chronic == 1:
    ...
    def parallel_simulation(self, medication_name: str, scenario: dict, group: str = 'Unique',
        ...
        return_list.to_csv(DIR_OUTPUT + 'results_' + medication_name + '_p_'+inflation_rate[0] + '_' +
    ...
    def simulate_change(self, state: dict, adverse_information: dict, scenario: dict, inflation_rate: float = 1.0):
        ...
        if random.random() < death_prob:
            ...
            qaly_cost = self.calculate_qaly_cost(viral_charge=True, age=age_group,
                                                 ...
                                                 step_length=time_for_step, adverse_information=adverse_information,
                                                 medication_name=medication_name)
            result_state['qaly'] = qaly_cost['qaly']*self.general_info['values']['switch_qaly']['ALL'][
