...
class PregnancyModel(object):
    ...
    def initial_state(self, efficiency: float, value: str = 'BASE_VALUE'):
        ...
        for age in self.age_groups:
            if random_value < 0.0:
                random_value -= self.general_info['values']['incidence'][age][value]
                if random_value <= 0.0:
    ...
    def calculate_probabilities(self, medication_name: str, scenario: dict):
        ...
        pregnancy = {'Immediate_Cost': 0, 'Immediate_QALY': 0}
        if probabilities is not None:
            ...
            for cond in pregnant_df['NAME'].unique():
                weights[cond] = pregnant_df[pregnant_df['NAME'] == cond]['PROBABILITY'].mean()
                probability *= (1 - weights[cond])
            probability = 1 - probability
            for cond in pregnant_df['NAME'].unique():
                if probability > 0:
    ...
    def simulate_medication(self, medication_name: str, scenario: dict, adverse_information: dict,
        ...
        if random.random() < adverse_information['pregnancy']['probability']:
    ...
    def calculate_qaly_cost(self, viral_charge: bool, age: str, chronic: int, acute: bool, dead: bool, step_length: int,
                            ...
                            integrate_pregnancy_risk: bool = False):
        """
        ...
        """
        result_vector = {'qaly': self.general_info['values']['base_qaly'][age][scenario.get('base_qaly',
                                                                                                   'BASE_VALUE')] / 12,
        ...
        if viral_charge:
            ...
            result_vector['test_cost'] += self.medications[medication_name]['values']['test_cost_high_charge']['ALL'][
        ...
        if integrate_pregnancy_risk:
            result_vector['qaly'] *= 1-adverse_information['pregnancy']['Immediate_QALY']
            if np.isnan(result_vector['qaly']) or result_vector['qaly'] > 1:
                print('integrate pregnancy risk')
            result_vector['pregnancy_risk_cost'] = adverse_information['pregnancy']['Immediate_Cost']
        if chronic == 1:
