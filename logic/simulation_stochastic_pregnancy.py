...
class Model(object):
    ...
    def calculate_qaly_cost(self, viral_charge: bool, age: str, chronic: int, acute: bool, step_length: int,
                            i: int, main_medication: bool = True, integrate_pregnancy_risk: bool = False):
        """
        :param integrate_pregnancy_risk: if there is a need to include the cost and QALY penalty from a pregnancy
        related event
        :param main_medication: boolean indicating if the value corresponds to main or switch phase state
        ...
        if integrate_pregnancy_risk:
            qaly *= 1-self.random_values['pregnancy']['Immediate_QALY'][i]
            cost += self.random_values['pregnancy']['Immediate_Cost'][i]
        if step_length > 6:
            cost = cost * 2
    ...
    def simulate_medication(self, result_list: list, iteration: int, inflation_rate: float,
        ...
        age_group = math.floor(state['age'] / 60)
        age_group = 'e' + str(min(age_group, 16)) if age_group >= 10 else 'e0' + str(age_group)
        if random.random() < self.random_values['pregnancy']['probability'][iteration]:
    ...
    def generate_random(self, n_iterations: int, medication_name: str):
        ...
        acute = {'Immediate_Death': np.zeros(n_iterations), 'Immediate_Change': np.zeros(n_iterations),
                 'Immediate_QALY': np.zeros(n_iterations), 'Immediate_Cost': np.zeros(n_iterations)}
        pregnancy = {'Immediate_QALY': np.zeros(n_iterations), 'Immediate_Cost': np.zeros(n_iterations)}
        if probabilities is not None:
            ...
            pregnancy_df = probabilities[probabilities['TYPE'] == 'G']
            probability = 1
            weights = dict()
            for cond in pregnancy_df['NAME'].unique():
                prob = pregnancy_df[pregnancy_df['NAME'] == cond]['PROBABILITY'].mean()
                if prob > 0:
                    weights[cond] = prob
                    probability *= (1 - weights[cond])
            probability = 1 - probability
